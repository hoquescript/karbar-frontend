...
const MenuButton = ({ type, gridSQL }) => {
    ...
    return (
        <Button
            variant="contained"
            style={{ ...style, backgroundColor: btnColorGenarator(type) }}
            onClick={handleSubmit(btnHandler)}
        >
            {type}
        </Button>
    );
...
const ActionButton = ({ menuButton, gridSQL }) => {
    const buttons = menuButton &&
        menuButton.map((btnType) => (
            <MenuButton key={btnType} type={btnType} gridSQL={gridSQL} />
        ));
